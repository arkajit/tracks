...
%emition probabilities E is a k x l matrix, but each row has to sum to 1 => k*(l-1) independent parameters
...
for i=1:length(X),
    ...
    log_lik = log_lik + log_sum_exp(log_a(:,end),1);
    ...
    for j=1:m-1,
        ...
        log_E = repmat(hmm.log_E(:,x(j+1)),1,k);
